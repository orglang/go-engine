...
func MsgToExpSpec(dto procexp.ExpSpec) (ExpSpec, error) {
	...
	case procexp.Lab:
		...
		return LabSpec{CommChnlPH: x, ValLabQN: label}, nil
	case procexp.Case:
		x, err := symbol.ConvertFromString(dto.Case.CommChnlPH)
		if err != nil {
			return nil, err
		}
		conts := make(map[uniqsym.ADT]ExpSpec, len(dto.Case.ContBSes))
	...
	case procexp.Call:
		...
		procQN, err := uniqsym.ConvertFromString(dto.Call.ProcDescQN)
		if err != nil {
			return nil, err
		}
		...
		return CallSpec{NewChnlPH: bindPH, ProcDescQN: procQN, ValChnlPHs: valPHs}, nil
	case procexp.Fwd:
...
func DataFromExpRec(r ExpRec) (ExpRecDS, error) {
	...
	case LabRec:
		return ExpRecDS{
			K:   labExp,
			Lab: &labRecDS{symbol.ConvertToString(rec.CommChnlPH), uniqsym.ConvertToString(rec.ValLabQN)},
		}, nil
	case CaseRec:
		brs := []branchRecDS{}
...
func DataToExpRec(dto ExpRecDS) (ExpRec, error) {
	...
	case labExp:
		...
		return LabRec{CommChnlPH: a, ValLabQN: label}, nil
	case caseExp:
...
func dataFromExpSpec(s ExpSpec) (ExpSpecDS, error) {
	...
	case LabSpec:
		return ExpSpecDS{
			K:   labExp,
			Lab: &labSpecDS{symbol.ConvertToString(spec.CommChnlPH), uniqsym.ConvertToString(spec.ValLabQN)},
		}, nil
	case CaseSpec:
		brs := []branchSpecDS{}
