package pool_test

import (
	"database/sql"
	"fmt"
	"os"
	"slices"
	"testing"

	_ "github.com/jackc/pgx/v5/stdlib"

	"orglang/orglang/adt/pooldec"
	"orglang/orglang/adt/poolexec"
	"orglang/orglang/adt/procdec"
	"orglang/orglang/adt/procexec"
	"orglang/orglang/adt/procexp"
	"orglang/orglang/adt/qualsym"
	"orglang/orglang/adt/termctx"
	"orglang/orglang/adt/typedef"
	"orglang/orglang/adt/typeexp"
)

var (
	poolDecAPI  = pooldec.NewAPI()
	poolExecAPI = poolexec.NewAPI()
	procDecAPI  = procdec.NewAPI()
	procExecAPI = procexec.NewAPI()
	typeDefAPI  = typedef.NewAPI()
	tc          *testCase
)

func TestMain(m *testing.M) {
	ts := testSuite{}
	tc = ts.Setup()
	code := m.Run()
	ts.Teardown()
	os.Exit(code)
}

type testSuite struct {
	db *sql.DB
}

func (ts *testSuite) Setup() *testCase {
	db, err := sql.Open("pgx", "postgres://orglang:orglang@localhost:5432/orglang")
	if err != nil {
		panic(err)
	}
	ts.db = db
	return &testCase{db}
}

func (ts *testSuite) Teardown() {
	err := ts.db.Close()
	if err != nil {
		panic(err)
	}
}

type testCase struct {
	db *sql.DB
}

func (tc *testCase) Setup(t *testing.T) {
	tables := []string{
		"aliases",
		"pool_roots", "pool_liabs", "proc_bnds", "proc_steps",
		"sig_roots", "sig_pes", "sig_ces",
		"type_def_roots", "type_term_states",
		"type_term_states"}
	for _, table := range tables {
		_, err := tc.db.Exec(fmt.Sprintf("truncate table %v", table))
		if err != nil {
			t.Fatal(err)
		}
	}
}

func TestCreation(t *testing.T) {

	t.Run("CreateRetreive", func(t *testing.T) {
		// given
		poolSpec1 := poolexec.ExecSpec{PoolQN: "ts1"}
		poolRef1, err := poolExecAPI.Create(poolSpec1)
		if err != nil {
			t.Fatal(err)
		}
		// and
		poolSpec2 := poolexec.ExecSpec{PoolQN: "ts2", SupID: poolRef1.ExecID}
		poolRef2, err := poolExecAPI.Create(poolSpec2)
		if err != nil {
			t.Fatal(err)
		}
		// when
		poolSnap1, err := poolExecAPI.Retrieve(poolRef1.ExecID)
		if err != nil {
			t.Fatal(err)
		}
		// then
		if !slices.Contains(poolSnap1.Subs, poolRef2) {
			t.Errorf("unexpected subs in %q; want: %+v, got: %+v",
				poolSpec1.PoolQN, poolRef2, poolSnap1.Subs)
		}
	})
}

func TestTaking(t *testing.T) {

	t.Run("WaitClose", func(t *testing.T) {
		tc.Setup(t)
		// given
		mainTypeSN := qualsym.New("main-type-sn")
		closerProcSN := qualsym.New("closer-proc-sn")
		waiterProcSN := qualsym.New("waiter-proc-sn")
		_, err := typeDefAPI.Create(typedef.DefSpec{
			TypeQN: mainTypeSN,
			TypeES: typeexp.UpSpec{
				Z: typeexp.XactSpec{
					Zs: map[qualsym.ADT]typeexp.ExpSpec{
						closerProcSN: typeexp.DownSpec{
							Z: typeexp.LinkSpec{TypeQN: mainTypeSN},
						},
						waiterProcSN: typeexp.DownSpec{
							Z: typeexp.LinkSpec{TypeQN: mainTypeSN},
						},
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		mainPoolSN := qualsym.New("main-pool-sn")
		mainProvisionPH := qualsym.New("main-provision-ph")
		mainReceptionPH := qualsym.New("main-reception-ph")
		_, err = poolDecAPI.Create(pooldec.DecSpec{
			PoolSN: mainPoolSN,
			InsiderProvisionEP: termctx.BindClaim{
				BindPH: mainProvisionPH,
				TypeQN: mainTypeSN,
			},
			InsiderReceptionEP: termctx.BindClaim{
				BindPH: mainReceptionPH,
				TypeQN: mainTypeSN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		mainExecRef, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: mainPoolSN,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneTypeSN := qualsym.New("one-type-sn")
		_, err = typeDefAPI.Create(typedef.DefSpec{
			TypeQN: oneTypeSN,
			TypeES: typeexp.OneSpec{},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		closerDecSpec := procdec.DecSpec{
			ProcQN: closerProcSN,
			ProvisionEP: termctx.BindClaim{
				BindPH: qualsym.New("closer-provision-ph"),
				TypeQN: oneTypeSN,
			},
		}
		_, err = procDecAPI.Create(closerDecSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		closerProcPH := qualsym.New("closer-proc-ph")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.AcqureSpec{
				CommPH: mainReceptionPH,
				ContES: procexp.CallSpec{
					BindPH: mainReceptionPH,
					CommPH: closerProcPH,
					ProcSN: closerProcSN,
					ContES: procexp.ReleaseSpec{
						CommPH: mainReceptionPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.AcceptSpec{
				CommPH: mainProvisionPH,
				ContES: procexp.SpawnSpec{
					CommPH: mainProvisionPH,
					ProcSN: closerProcSN,
					ContES: procexp.DetachSpec{
						CommPH: mainProvisionPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		closerExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		waiterDecSpec := procdec.DecSpec{
			ProcQN:      waiterProcSN,
			ProvisionEP: termctx.BindClaim{BindPH: qualsym.New("waiter-provision-ph"), TypeQN: oneTypeSN},
			ReceptionEPs: []termctx.BindClaim{
				{BindPH: qualsym.New("closer-reception-ph"), TypeQN: oneTypeSN},
			},
		}
		_, err = procDecAPI.Create(waiterDecSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.AcqureSpec{
				CommPH: mainProvisionPH,
				ContES: procexp.CallSpec{
					BindPH: mainProvisionPH,
					CommPH: qualsym.New("waiter-proc-ph"),
					ProcSN: waiterProcSN,
					ValPHs: []qualsym.ADT{closerProcPH},
					ContES: procexp.ReleaseSpec{
						CommPH: mainProvisionPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.AcceptSpec{
				CommPH: mainProvisionPH,
				ContES: procexp.SpawnSpec{
					CommPH: mainProvisionPH,
					ProcSN: waiterProcSN,
					ContES: procexp.DetachSpec{
						CommPH: mainProvisionPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		waiterExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// when
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: closerExecRef.ExecID,
			ProcES: procexp.CloseSpec{
				CommPH: closerDecSpec.ProvisionEP.BindPH,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: waiterExecRef.ExecID,
			ProcES: procexp.WaitSpec{
				CommPH: waiterDecSpec.ReceptionEPs[0].BindPH,
				ContES: procexp.CloseSpec{
					CommPH: waiterDecSpec.ProvisionEP.BindPH,
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// then
		// TODO добавить проверку
	})

	t.Run("RecvSend", func(t *testing.T) {
		tc.Setup(t)
		// given
		mainTypeSN := qualsym.New("main-type-sn")
		senderProcSN := qualsym.New("sender-proc-sn")
		receiverProcSN := qualsym.New("receiver-proc-sn")
		messageProcSN := qualsym.New("message-proc-sn")
		_, err := typeDefAPI.Create(typedef.DefSpec{
			TypeQN: mainTypeSN,
			TypeES: typeexp.UpSpec{
				Z: typeexp.XactSpec{
					Zs: map[qualsym.ADT]typeexp.ExpSpec{
						senderProcSN: typeexp.DownSpec{
							Z: typeexp.LinkSpec{TypeQN: mainTypeSN},
						},
						receiverProcSN: typeexp.DownSpec{
							Z: typeexp.LinkSpec{TypeQN: mainTypeSN},
						},
						messageProcSN: typeexp.DownSpec{
							Z: typeexp.LinkSpec{TypeQN: mainTypeSN},
						},
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		mainPoolSN := qualsym.New("main-pool-sn")
		mainProvisionPH := qualsym.New("main-provision-ph")
		mainReceptionPH := qualsym.New("main-reception-ph")
		_, err = poolDecAPI.Create(pooldec.DecSpec{
			PoolSN: mainPoolSN,
			InsiderProvisionEP: termctx.BindClaim{
				BindPH: mainProvisionPH,
				TypeQN: mainTypeSN,
			},
			InsiderReceptionEP: termctx.BindClaim{
				BindPH: mainReceptionPH,
				TypeQN: mainTypeSN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		mainExecRef, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: mainPoolSN,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		lolliTypeSN := qualsym.New("lolli-type-sn")
		_, err = typeDefAPI.Create(typedef.DefSpec{
			TypeQN: lolliTypeSN,
			TypeES: typeexp.LolliSpec{
				Y: typeexp.OneSpec{},
				Z: typeexp.OneSpec{},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneTypeSN := qualsym.New("one-type-sn")
		_, err = typeDefAPI.Create(typedef.DefSpec{
			TypeQN: oneTypeSN,
			TypeES: typeexp.OneSpec{},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		receiverDecSpec := procdec.DecSpec{
			ProcQN: receiverProcSN,
			ProvisionEP: termctx.BindClaim{
				BindPH: qualsym.New("receiver-provision-ph"),
				TypeQN: lolliTypeSN,
			},
		}
		_, err = procDecAPI.Create(receiverDecSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		messageDecSpec := procdec.DecSpec{
			ProcQN: messageProcSN,
			ProvisionEP: termctx.BindClaim{
				BindPH: qualsym.New("message-provision-ph"),
				TypeQN: oneTypeSN,
			},
		}
		_, err = procDecAPI.Create(messageDecSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		senderDecSpec := procdec.DecSpec{
			ProcQN: senderProcSN,
			ProvisionEP: termctx.BindClaim{
				BindPH: qualsym.New("sender-provision-ph"),
				TypeQN: oneTypeSN,
			},
			ReceptionEPs: []termctx.BindClaim{
				{BindPH: qualsym.New("receiver-reception-ph"), TypeQN: lolliTypeSN},
				{BindPH: qualsym.New("message-reception-ph"), TypeQN: oneTypeSN},
			},
		}
		_, err = procDecAPI.Create(senderDecSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		receiverProcPH := qualsym.New("receiver-proc-ph")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: mainReceptionPH,
				CommPH: receiverProcPH,
				ProcSN: receiverProcSN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		messageProcPH := qualsym.New("message-proc-ph")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: mainReceptionPH,
				CommPH: messageProcPH,
				ProcSN: messageProcSN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		senderProcPH := qualsym.New("sender-proc-ph")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: mainReceptionPH,
				CommPH: senderProcPH,
				ProcSN: senderProcSN,
				ValPHs: []qualsym.ADT{receiverProcPH, messageProcPH},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		receiverExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// when
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: receiverExecRef.ExecID,
			ProcES: procexp.RecvSpec{
				BindPH: receiverDecSpec.ProvisionEP.BindPH,
				CommPH: qualsym.New("message-reception-ph"),
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		senderExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: senderExecRef.ExecID,
			ProcES: procexp.SendSpec{
				CommPH: senderDecSpec.ReceptionEPs[0].BindPH,
				ValPH:  senderDecSpec.ReceptionEPs[1].BindPH,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// then
		// TODO добавить проверку
	})

	t.Run("CaseLab", func(t *testing.T) {
		tc.Setup(t)
		// given
		mainPoolSN := qualsym.New("main-pool-sn")
		mainExecRef, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: mainPoolSN,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		label := qualsym.ADT("label-1")
		// and
		withRoleSpec := typedef.DefSpec{
			TypeQN: "with-role",
			TypeES: typeexp.WithSpec{
				Zs: map[qualsym.ADT]typeexp.ExpSpec{
					label: typeexp.OneSpec{},
				},
			},
		}
		withRole, err := typeDefAPI.Create(withRoleSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneRoleSpec := typedef.DefSpec{
			TypeQN: "one-role",
			TypeES: typeexp.OneSpec{},
		}
		oneRole, err := typeDefAPI.Create(oneRoleSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		withSigSpec := procdec.DecSpec{
			ProcQN: "sig-1",
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-1",
				TypeQN: withRole.TypeQN,
			},
		}
		withSig, err := procDecAPI.Create(withSigSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneSigSpec := procdec.DecSpec{
			ProcQN:       "sig-2",
			ReceptionEPs: []termctx.BindClaim{withSig.X},
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-2",
				TypeQN: oneRole.TypeQN,
			},
		}
		_, err = procDecAPI.Create(oneSigSpec)
		if err != nil {
			t.Fatal(err)
		}
		// and
		followerPH := qualsym.New("follower")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: followerPH,
				ProcSN: "tbd",
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		deciderPH := qualsym.New("decider")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: deciderPH,
				ProcSN: "tbd",
				ValPHs: []qualsym.ADT{followerPH},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		followerExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// when
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: followerExecRef.ExecID,
			ProcES: procexp.CaseSpec{
				CommPH: followerPH,
				ContESs: map[qualsym.ADT]procexp.ExpSpec{
					label: procexp.CloseSpec{
						CommPH: followerPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		deciderExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: deciderExecRef.ExecID,
			ProcES: procexp.LabSpec{
				CommPH: followerPH,
				Label:  label,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// then
		// TODO добавить проверку
	})

	t.Run("Spawn", func(t *testing.T) {
		tc.Setup(t)
		// given
		mainPoolSN := qualsym.New("main-pool-sn")
		mainExecRef, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: mainPoolSN,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneRole, err := typeDefAPI.Create(
			typedef.DefSpec{
				TypeQN: "one-role",
				TypeES: typeexp.OneSpec{},
			},
		)
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneSig1, err := procDecAPI.Create(procdec.DecSpec{
			ProcQN: "sig-1",
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-1",
				TypeQN: oneRole.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		_, err = procDecAPI.Create(procdec.DecSpec{
			ProcQN:       "sig-2",
			ReceptionEPs: []termctx.BindClaim{oneSig1.X},
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-2",
				TypeQN: oneRole.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneSig3, err := procDecAPI.Create(procdec.DecSpec{
			ProcQN:       "sig-3",
			ReceptionEPs: []termctx.BindClaim{oneSig1.X},
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-3",
				TypeQN: oneRole.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		poolImpl, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: "pool-1",
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		injecteePH := qualsym.New("injectee")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: poolImpl.ExecID,
			ExecID: poolImpl.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: injecteePH,
				ProcSN: "tbd",
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		spawnerPH := qualsym.New("spawner")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: poolImpl.ExecID,
			ExecID: poolImpl.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: spawnerPH,
				ProcSN: "tbd",
				ValPHs: []qualsym.ADT{injecteePH},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		x := qualsym.New("x")
		// and
		spawnerExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// when
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: poolImpl.ExecID,
			ExecID: spawnerExecRef.ExecID,
			ProcES: procexp.SpawnSpecOld{
				SigID: oneSig3.DecID,
				Ys:    []qualsym.ADT{injecteePH},
				X:     x,
				ContES: procexp.WaitSpec{
					CommPH: x,
					ContES: procexp.CloseSpec{
						CommPH: spawnerPH,
					},
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// then
		// TODO добавить проверку
	})

	t.Run("Fwd", func(t *testing.T) {
		tc.Setup(t)
		// given
		mainPoolQN := qualsym.New("main-pool-qn")
		mainExecRef, err := poolExecAPI.Create(poolexec.ExecSpec{
			PoolQN: mainPoolQN,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneDefSnap, err := typeDefAPI.Create(typedef.DefSpec{
			TypeQN: "one-role",
			TypeES: typeexp.OneSpec{},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		oneDecSnap, err := procDecAPI.Create(procdec.DecSpec{
			ProcQN: "sig-1",
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-1",
				TypeQN: oneDefSnap.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		_, err = procDecAPI.Create(procdec.DecSpec{
			ProcQN:       "sig-2",
			ReceptionEPs: []termctx.BindClaim{oneDecSnap.X},
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-2",
				TypeQN: oneDefSnap.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		_, err = procDecAPI.Create(procdec.DecSpec{
			ProcQN:       "sig-3",
			ReceptionEPs: []termctx.BindClaim{oneDecSnap.X},
			ProvisionEP: termctx.BindClaim{
				BindPH: "chnl-3",
				TypeQN: oneDefSnap.TypeQN,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		closerChnlPH := qualsym.New("closer")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: closerChnlPH,
				ProcSN: "tbd",
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		forwarderChnlPH := qualsym.New("forwarder")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: forwarderChnlPH,
				ProcSN: "tbd",
				ValPHs: []qualsym.ADT{closerChnlPH},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		waiterChnlPH := qualsym.New("waiter")
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: mainExecRef.ProcID,
			ProcES: procexp.CallSpec{
				BindPH: waiterChnlPH,
				ProcSN: "tbd",
				ValPHs: []qualsym.ADT{forwarderChnlPH},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		closerExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: closerExecRef.ExecID,
			ProcES: procexp.CloseSpec{
				CommPH: closerChnlPH,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		forwarderExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// when
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: forwarderExecRef.ExecID,
			ProcES: procexp.FwdSpec{
				X: forwarderChnlPH,
				Y: closerChnlPH,
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		waiterExecRef, err := poolExecAPI.Poll(poolexec.PollSpec{
			PoolID: mainExecRef.ExecID,
		})
		if err != nil {
			t.Fatal(err)
		}
		// and
		err = procExecAPI.Run(procexec.ExecSpec{
			PoolID: mainExecRef.ExecID,
			ExecID: waiterExecRef.ExecID,
			ProcES: procexp.WaitSpec{
				CommPH: forwarderChnlPH,
				ContES: procexp.CloseSpec{
					CommPH: waiterChnlPH,
				},
			},
		})
		if err != nil {
			t.Fatal(err)
		}
		// then
		// TODO добавить проверку
	})
}
